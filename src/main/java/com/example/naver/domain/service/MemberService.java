...
public class MemberService {
    ...
    public Member findByIdWithCouple(Long id) {
        ...
        if (member.getNickname() == null) {
            throw new MemberException(DELETED_MEMBER.getCode(), DELETED_MEMBER.getErrorMessage());
        }
    ...
    public void findByPhone(String phoneNumber) {
        ...
        if (findAuth.isPresent()) {
            ...
            if (auth.getDeleteDate() != null) {
                throw new MemberException(DELETED_PHONE_NUMBER.getCode(), DELETED_PHONE_NUMBER.getErrorMessage());
            }

            throw new MemberException(PHONE_INFO_EXIST.getCode(), PHONE_INFO_EXIST.getErrorMessage());
        }
    }
     ...
     * 회원저장 - 애플
     * */
    public Member createMember(MemberAppleCreateRequestDto dto) {
        findByPhone(dto.getPhone());
        Auth auth = new Auth(dto.getPhone());
